...
struct FormFeelingView: View {
    ...
    var activities: [String] = ["Working", "Driving", "Gym", "Cleaning", "Eating"]
    @State var selectedActivity: String = "Working"
    var places: [String] = ["Outside", "Office", "Commuting", "Home"]
    @State var selectedPlace: String = "Outside"
    @State var routeToSuccessSubmit: Bool = false

    var body: some View {
        VStack (spacing: 0){
            NavigationLink(
                            destination: SuccessSubmitFeelingView(),
                            isActive: $routeToSuccessSubmit) {}
            ScrollView {
                VStack {
                    VStack {
